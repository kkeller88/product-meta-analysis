...
class FuzzyBrandRecognizer(BrandRecognizerBase):
    def __init__(self, brands):
        ...
        self.n_grams = 3
        self.brands_ngrams = {x:get_ngram_strings(x, self.n_grams) for x in self.brands}
    ...
    def _get_brand(self, token:str):
        ...
        if n_char >= self.min_token_length:
            token_ngrams = get_ngram_strings(token.text, self.n_grams)
            scores = {
                k:get_jaccard_dist(token_ngrams, v)
...
def get_ngram_strings(s, n):
    return [''.join(x).lower() for x in ngrams(s, n)]

def get_jaccard_dist(list1, list2):
    s1 = set(list1)
    s2 = set(list2)
